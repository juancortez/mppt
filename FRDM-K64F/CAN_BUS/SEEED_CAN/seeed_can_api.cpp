...
#ifdef DEBUG
    udebug.printf("Reseting MCP2515\r\n");
#endif
    mcpReset(obj);

    for (uint32_t i = 0; i < 8; i++) {                                  // Clear all CAN id masks and filters
        mcpWriteId(obj, maskFilt[i], 0, 0);
    }
    for (uint32_t i = 0; i < 5; i++) {                                  // Clear all CAN buffer control registers
        mcpWrite(obj, canBufCtrl[i], 0);
    }
    for (uint32_t i = 0; i < sizeof(x); i++) y[i] = 0;                  // Initialise empty CAN message buffer
...
#ifdef DEBUG
    udebug.printf("Setting bit rate\r\n");
#endif
...
#ifdef DEBUG
    udebug.printf("Failed to enter mode: %02x\r\n", newmode);
    udebug.printf("CANCTRL:%02x CANSTAT:%02x TXB0:%02x TXB1:%02x TXB2:%02x\r\n", mcpRead(obj, MCP_CANCTRL),  mcpRead(obj, MCP_CANSTAT), mcpRead(obj, MCP_TXB0CTRL), mcpRead(obj, MCP_TXB1CTRL), mcpRead(obj, MCP_TXB2CTRL));
#endif
    return 0;
}
...
uint8_t mcpSetBitRate(mcp_can_t *obj, const uint32_t bitRate)
{
    union {                                                             // Access CANtiming as:
        CANtiming x;                                                    // the organized struct
        uint8_t y[];                                                    // or contiguous memory array
    };
    uint32_t bestBRP = 0;
    uint32_t bestTQU = 0;
...
#ifdef DEBUG
    udebug.printf("Setting configuration mode\r\n");
#endif
    ...
    for (uint32_t i = 0; i < sizeof(x); i++) y[i] = 0;               // Initialise CANtiming (btlmode, sjw and sam all = 0)
    if ((bitRate < CAN_MIN_RATE) || (bitRate > CAN_MAX_RATE)) {
#ifdef DEBUG
        udebug.printf("FAILED!! The requested Bit Rate is too high or too low: %d\r\n", bitRate);
#endif
...
#ifdef DEBUG
    udebug.printf("minBRP %d maxBRP %d\r\n", minBRP, maxBRP);
    udebug.printf("Bitrate: %d\tactualBitRate: %d\t Error: %1.2f percent.\r\n", bitRate, bestCanRate, (100-(100*(float)bitRate/(float)bestCanRate)));
    udebug.printf("TimeQuanta: %d\tbitRatePrescaler: %d\tSamplePoint: %2.2f percent\r\n", bestTQU, bestBRP, 100*(float)(3 + x.prseg + x.phseg1)/(float)bestTQU ) ;
    udebug.printf("Syncseg: 1\tPropSeg: %d\tPhaseSeg1: %d\tPhaseSeg2: %d\r\n", (x.prseg+1), (x.phseg1+1), (x.phseg1+1));
    udebug.printf("Setting normal mode\r\n");
#endif
...
void mcpWriteId(mcp_can_t *obj, const uint8_t mcp_addr, const uint8_t ext, const uint32_t id )
{
    ...
    for (uint32_t i = 0; i < sizeof(x); i++) y[i] = 0;               // Initialise CANid structure
...
#ifdef DEBUG
    udebug.printf("sizeof CanIdStruct: %d bytes\r\n", sizeof(x));
    udebug.printf("sid10_3: %x\r\n", x.sid10_3);
    udebug.printf("eid17_16: %x\r\n", x.eid17_16);
    udebug.printf("ide: %x\r\n", x.ide);
    udebug.printf("srtr: %x\r\n", x.srtr);
    udebug.printf("sid2_0: %x\r\n", x.sid2_0);
    udebug.printf("eid15_8: %x\r\n", x.eid15_8);
    udebug.printf("eid7_0: %x\r\n", x.eid7_0);
#endif
...
// populate CANMsg structure
    for (uint32_t i = 0; i < sizeof(x); i++) y[i] = 0;               // Initialise CANMsg structure
...
// Check if there is a message the buffers
	...
	mcpBitModify(obj, MCP_CANINTF, (!num ? MCP_RX0IF : MCP_RX1IF), 0);  // clear interrupt anyway  JCW !!!!!
...
#ifdef DEBUG
    udebug.printf("sizeof CanMsgStruct: %d bytes\r\n", sizeof(x));
    udebug.printf("sizeof CanMsgArray: %d bytes\r\n", sizeof(y));
    udebug.printf("sid10_3: %x\r\n", x.id.sid10_3);
    udebug.printf("eid17_16: %x\r\n", x.id.eid17_16);
    udebug.printf("ide: %x\r\n", x.id.ide);
    udebug.printf("srtr: %x\r\n", x.id.srtr);
    ...
    udebug.printf("eid7_0: %x\r\n", x.id.eid7_0);
    udebug.printf("dlc: %x\r\n", x.dlc);
    udebug.printf("ertr: %x\r\n", x.ertr);
    udebug.printf("data: ");
    for (char i=0; i<8; i++)
        udebug.printf("%02x,", x.data[i]);
    udebug.printf("\r\n");
#endif
...
uint8_t mcpInitMask(mcp_can_t *obj, uint8_t num, uint32_t ulData, bool ext)
{
    ...
    if (num > 1) {
#ifdef DEBUG
        udebug.printf("Trying to set an invalid Mask number: %d\r\n", num);
#endif
        return 0;
    }
#ifdef DEBUG
    udebug.printf("Begin to set Mask!!\r\n");
#endif
    uint8_t initialMode = mcpRead(obj, MCP_CANCTRL) & MODE_MASK;        // Store the current operation mode
...
#ifdef DEBUG
    udebug.printf("Successfully set Mask number: %d\r\n", num);
#endif
    return 1;
}
...
uint8_t mcpInitFilter(mcp_can_t *obj, uint8_t num, uint32_t ulData, bool ext)
{
    ...
    if (num > 5) {
#ifdef DEBUG
        udebug.printf("Trying to set an invalid Filter number: %d\r\n", num);
#endif
        return 0;
    }
#ifdef DEBUG
    udebug.printf("Begin to set Filter!!\r\n");
#endif
    uint8_t initialMode = mcpRead(obj, MCP_CANCTRL) & MODE_MASK;        // Store the current operation mode
    if(!mcpSetMode(obj, MODE_CONFIG)) {
        return 0;
...
#ifdef DEBUG
    udebug.printf("Successfully set Filter: %d\r\n", num);
#endif
    return 1;
}
